...
Update the existing ones to `0.5.x` and install new packages:

```shell
npm i --save @ui-schema/react @ui-schema/react-json-schema @ui-schema/json-schema @ui-schema/json-pointer
```

- React code moved to `/react` and `/react-json-schema` respectively, all other packages are isomorphic
- validator related code moved to `/json-schema`
- React code which implements specific JSON Schema handling moved to `/react-json-schema`, while the code in `/react` is agnostic to the schema structure itself
