...
- React code was moved to `/react` and `/react-json-schema` respectively, all other packages are isomorphic
- validator related code was moved to `/json-schema`
- React code which implements specific JSON Schema handling was moved to `/react-json-schema`, while the code in `/react` is agnostic to the schema structure itself
