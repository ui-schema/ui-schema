...
if (typeof createMap === 'function') {
    console.log('✅ -cjs.js resolved createMap as function')
} else {
    throw new Error(`-cjs.js resolved wrong symbol: ${typeof createMap}`)
}
...
for (const [specifier, expectedPath] of expected) {
    ...
    if (!resolved.endsWith('/node_modules/' + expectedPath)) {
        throw new Error(`-cjs.js resolved to unexpected file for ${specifier}: ${resolved}`)
    } else {
        console.log(`✅ -cjs.js resolved ${specifier} to expected file: ${resolved}`)
    }
}
